...
export function ProductCard({ product }: { product: ProductSummary }) {
  ...
  function buyNow() {
    ...
    const checkoutPath = `/checkout?buyNow=${encodeURIComponent(
      product.slug,
    )}&quantity=1`
  ...
  return (
    <article
      className={`catalog-card ${
        secondHandOutOfStock ? 'relative overflow-hidden' : ''
      }`}
    >
      {secondHandOutOfStock ? (
        <div
      ...
      {secondHandOutOfStock ? (
        <div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center bg-black/45 backdrop-blur-[2px]">
          <span className="rounded-full border border-red-400/50 bg-red-500/90 px-5 py-2 text-sm font-extrabold tracking-[0.18em] text-white shadow-xl">
            OUT OF STOCK
          </span>
        </div>
      ) : null}
