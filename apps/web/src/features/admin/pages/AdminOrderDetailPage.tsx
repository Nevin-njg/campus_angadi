...
export function AdminOrderDetailPage() {
  ...
  if (!order.data || order.isError) return (
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Link className="inline-flex items-center text-sm font-medium text-gray-400 hover:text-white transition-colors" to="/admin/orders">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-lg">{new Date(data.createdAt).toLocaleString('en-IN', {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-6 pb-4 border-b border-white/10">Products</h2>
            <div className="space-y-4">
              {data.items.map((item) => (
                <div className="flex items-center gap-4 p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-colors" key={item.id}>
                  {item.productImageUrl ? (
                    <img src={item.productImageUrl} alt="" className="w-16 h-16 rounded-lg object-cover bg-black/40 border border-white/10" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <strong className="text-white font-medium text-lg block truncate">{item.productName}</strong>
                    <small className="text-gray-400 block mt-1">
                      Quantity <span className="text-white">{item.quantity}</span> · {price(item.unitPrice)} each
                    </small>
                  </div>
                  <span className="text-indigo-400 font-bold bg-indigo-500/10 px-3 py-1 rounded-full">{price(item.totalPrice)}</span>
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-white/10 flex items-center justify-between">
              ...
              <strong className="text-2xl font-extrabold text-white">{price(data.totalAmount)}</strong>
            </div>
          </section>
          
          <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-6 pb-4 border-b border-white/10">Status history</h2>
            <div className="relative pl-6 space-y-6 before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
              {data.statusHistory.map((entry, i) => (
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active" key={entry.id}>
                  ...
                  <div className="bg-white/[0.02] border border-white/5 p-4 rounded-xl w-full">
                    <div className="flex items-center justify-between mb-2">
                      <strong className="text-white font-medium">{entry.toStatus.replaceAll('_', ' ')}</strong>
                      <small className="text-gray-400 text-xs px-2 py-0.5 bg-black/40 rounded-full">{new Date(entry.createdAt).toLocaleString('en-IN')}</small>
                    </div>
                    {entry.note && (
                      <p className="text-sm text-gray-300 bg-black/20 p-2 rounded-lg border border-white/5 inline-block">{entry.note}</p>
                    )}
                  </div>
                </div>
              ))}
          ...
          <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-6 pb-4 border-b border-white/10">Dealer assignment history</h2>
                    ...
                    <div className="bg-white/[0.02] border border-white/5 p-4 rounded-xl w-full">
                      <div className="flex items-center justify-between mb-2">
                        <strong className="text-white font-medium">
                          {entry.newDealer ? `Assigned to ${entry.newDealer.displayName}` : 'Unassigned'}
                        </strong>
        ...
        <aside className="space-y-8 sticky top-8">
          <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-6 pb-4 border-b border-white/10">Buyer and pickup</h2>
            <dl className="space-y-4">
              ...
              <div className="flex flex-col gap-1">
                <dt className="text-xs uppercase tracking-wider text-gray-500 font-medium">Phone</dt>
              ...
              {data.campusId && (
                <div className="flex flex-col gap-1">
                  <dt className="text-xs uppercase tracking-wider text-gray-500 font-medium">Campus ID</dt>
                  <dd className="text-white font-medium">{data.campusId}</dd>
                </div>
              )}
              {data.department && (
                <div className="flex flex-col gap-1">
                  <dt className="text-xs uppercase tracking-wider text-gray-500 font-medium">Department</dt>
                  <dd className="text-white font-medium">{data.department}</dd>
                </div>
              )}
              {data.notes && (
                <div className="flex flex-col gap-1">
                  <dt className="text-xs uppercase tracking-wider text-gray-500 font-medium">Buyer note</dt>
                  <dd className="text-white bg-black/20 p-3 rounded-lg border border-white/5 mt-1 text-sm">{data.notes}</dd>
                </div>
              )}
            </dl>
          </section>
          ...
          <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-6 pb-4 border-b border-white/10">WhatsApp dealer</h2>
            ...
            {data.assignedDealer ? (
              <div className="flex items-center gap-4 bg-green-500/10 border border-green-500/20 p-4 rounded-xl mb-6">
                ...
                <div>
                  <strong className="text-white block font-medium">{data.assignedDealer.displayName}</strong>
            ...
            {!['COMPLETED', 'CANCELLED', 'REJECTED'].includes(data.status) && (
              <div className="space-y-4">
                <label className={labelClass}>
                  ...
                  >
                    ...
                    {dealers.data?.items.map((dealer) => (
                      ...
                      >
                        {dealer.displayName} ({dealer.currentOpenOrders}/{dealer.maxOpenOrders} open)
                      </option>
                    ))}
                  </select>
                </label>
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    className="flex-1 px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg transition-all shadow-[0_0_15px_rgba(79,70,229,0.3)] disabled:opacity-50 disabled:cursor-not-allowed"
                    disabled={!dealerId || assign.isPending}
                    onClick={() => assign.mutate('MANUAL')}
                  >
                    Assign selected
                  </button>
                  <button
          ...
          {nextStatuses[data.status].length > 0 && (
            <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden">
              ...
              <h2 className="text-xl font-bold text-white mb-6 pb-4 border-b border-white/10 relative z-10">Update status</h2>
