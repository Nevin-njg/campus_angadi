      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-lg">
            Completed revenue is separated from pending and cancelled order value.
          </p>
        </div>
        <select
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
              ...
              <span className="text-gray-400 text-sm font-medium uppercase tracking-wider block mb-2">
                Order value
              </span>
              <strong className="text-3xl font-extrabold text-white block mb-1">
                {money(d.totals.orderValue)}
              </strong>
              <small className="text-amber-400 font-medium">All created orders</small>
            </article>
            <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
              <div className="absolute inset-0 bg-gradient-to-br from-amber-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
              <span className="text-gray-400 text-sm font-medium uppercase tracking-wider block mb-2">
              ...
              <strong className="text-3xl font-extrabold text-white block mb-1">
                {money(d.totals.confirmedValue)}
              </strong>
              <small className="text-amber-400 font-medium">Confirmed or later</small>
            </article>
            <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
              ...
              <strong className="text-3xl font-extrabold text-green-400 block mb-1">
                {money(d.totals.completedValue)}
              </strong>
              <small className="text-green-500/80 font-medium">
                {d.totals.completedOrders} completed orders
              </small>
            </article>
            <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
            ...
            <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
              ...
              <strong className="text-3xl font-extrabold text-white block mb-1">
                {money(d.totals.averageOrderValue)}
              </strong>
              <small className="text-amber-400 font-medium">Completed orders only</small>
            </article>
            <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
              ...
              <strong className="text-3xl font-extrabold text-white block mb-1">
                {money(d.bySellerType.secondHand)}
              </strong>
              <small className="text-orange-400 font-medium">
                Official: {money(d.bySellerType.official)}
              </small>
            </article>
          </div>
            ...
            <div className="space-y-4">
              {d.timeline.map((x) => (
                <div className="flex items-center gap-4" key={x.label}>
                  <span className="w-24 text-sm text-gray-400 text-right whitespace-nowrap">
                    {x.label}
                  </span>
                  <div className="flex-1 h-3 bg-black/40 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-amber-500 rounded-full shadow-[0_0_10px_rgba(245,158,11,0.35)] transition-all duration-1000"
                      style={{
                        width: `${Math.max(3, d.totals.completedValue ? (x.completedValue / d.totals.completedValue) * 100 : 0)}%`,
                      }}
                    />
                  </div>
                  <strong className="w-24 text-right text-white font-medium">
                    {money(x.completedValue)}
                  </strong>
                </div>
              ))}
            </div>
              ...
              <div className="space-y-3">
                {d.dealerPerformance.map((x) => (
                  ...
                  >
                    ...
                    <span className="text-green-400 font-bold bg-green-500/10 px-3 py-1 rounded-full">
                      {money(x.completedValue)}
                    </span>
                  </div>
                ))}
                {!d.dealerPerformance.length && (
                  <div className="text-center py-6 text-gray-400">
                    No dealer performance for this period.
                  </div>
                )}
              </div>
            </section>
