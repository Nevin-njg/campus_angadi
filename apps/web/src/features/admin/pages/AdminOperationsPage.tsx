    ...
    <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
          ...
          <strong
            className={`text-2xl font-bold block mb-1 flex items-center gap-2 ${value.mongoReady ? 'text-emerald-400' : 'text-red-400'}`}
          >
            {value.mongoReady ? (
        ...
        <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
          ...
          <strong
            className={`text-2xl font-bold block mb-1 flex items-center gap-2 ${value.redisReady ? 'text-emerald-400' : value.redisRequired ? 'text-red-400' : 'text-amber-400'}`}
          >
            {value.redisReady ? (
              <CheckCircleIcon className="w-6 h-6" />
            ) : (
          ...
          <small className="text-gray-500">Rate limits and job locks</small>
        </article>
        ...
        <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
          ...
          <strong
            className={`text-2xl font-bold block mb-1 ${value.cleanupEnabled ? 'text-white' : 'text-gray-500'}`}
          >
            {value.cleanupEnabled ? `${value.cleanupIntervalMinutes} min` : 'Disabled'}
          </strong>
          <small className={value.cleanupRunning ? 'text-amber-400' : 'text-gray-500'}>
            {value.cleanupRunning ? 'A cleanup run is active' : 'No cleanup currently running'}
          </small>
        </article>

        <article
          className={`backdrop-blur-xl rounded-2xl p-6 shadow-xl relative overflow-hidden group border ${value.indexDriftCount > 0 ? 'bg-red-500/5 border-red-500/20' : 'bg-white/5 border-white/10'}`}
        >
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
          ...
          <strong
            className={`text-2xl font-bold block mb-1 ${value.indexDriftCount > 0 ? 'text-red-400' : 'text-white'}`}
          >
      ...
      {cleanup.data ? (
        <section
          className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-8 shadow-xl relative overflow-hidden"
          aria-live="polite"
        >
          ...
          <div className="relative z-10">
            <div className="flex items-start gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-gray-400 text-lg">
                  <span className="text-white font-medium">{count(cleanup.data)}</span> records or
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <div className="bg-black/40 border border-white/5 rounded-xl p-4 flex flex-col justify-between h-24">
                <dt className="text-sm text-gray-400 font-medium leading-tight">
      ...
      <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-8 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-4 border-b border-white/10">
          <div>
            ...
            <p className="text-gray-400">
              Verifying that database indexes match application schemas.
            </p>
          </div>
          <button
          ...
          >
            <RefreshCwIcon
              className={`w-4 h-4 ${indexes.isFetching ? 'animate-spin text-amber-400' : ''}`}
            />{' '}
            Refresh
          </button>
        </div>
        ...
        {indexes.isLoading ? (
          <div className="py-12 flex flex-col items-center justify-center text-gray-400">
            <RefreshCwIcon className="w-8 h-8 animate-spin text-amber-500/50 mb-4" />
        ...
        {indexes.data?.length === 0 ? (
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-6 text-center">
            ...
            <strong className="text-emerald-400 font-medium text-lg block">
              No index drift detected
            </strong>
            <p className="text-gray-400 mt-1">
              All database indexes are properly synchronized with application schemas.
            </p>
          </div>
        ) : null}
                ...
                <div className="space-y-2">
                  {item.toCreate.length ? (
                    <div className="flex items-start gap-3">
                      <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 text-xs font-bold uppercase rounded shrink-0 mt-0.5">
                        Create
                      </span>
                  ...
                  {item.toDrop.length ? (
                    <div className="flex items-start gap-3">
                      <span className="px-2 py-1 bg-red-500/20 text-red-400 text-xs font-bold uppercase rounded shrink-0 mt-0.5">
                        Drop
                      </span>
                      <p className="text-gray-300 font-mono text-sm break-all">
                        {item.toDrop.join(', ')}
