    ...
    <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-lg">Add official products and review the current catalogue.</p>
        </div>
      </div>

      <details className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-xl group [&_summary::-webkit-details-marker]:hidden" open>
        ...
        <form className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={submit}>
          ...
          <div className="md:col-span-2">
            {message ? <p className="mb-4 text-green-400 p-3 bg-green-500/10 border border-green-500/20 rounded-lg">{message}</p> : null}
            <button 
              className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl transition-all shadow-[0_0_15px_rgba(79,70,229,0.3)] disabled:opacity-50 disabled:cursor-not-allowed" 
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          ...
          <span className="bg-white/10 text-white px-3 py-1 rounded-full text-sm font-medium">{products.data?.meta.total ?? 0} products</span>
        </div>
        
        <div className="p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {(products.data?.items ?? []).map((product) => (
              <article className="flex items-center gap-4 p-4 rounded-xl hover:bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group" key={product.id}>
                ...
                <div className="flex-1 min-w-0">
                  <strong className="text-white font-medium block truncate text-lg">{product.title}</strong>
                  <div className="text-gray-400 text-sm mt-1">
                    <span className="text-indigo-400">{product.category.name}</span> · <span className="font-medium text-white">₹{product.price.toLocaleString('en-IN')}</span> · {product.stock} in stock
                  </div>
                  <div className="flex gap-2 mt-2">
                    <span className={`text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded border ${product.published ? 'bg-green-500/20 text-green-400 border-green-500/20' : 'bg-gray-500/20 text-gray-400 border-gray-500/20'}`}>
                      {product.published ? 'Published' : 'Unpublished'}
                    </span>
                    <span className={`text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded border ${product.isFeatured ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/20' : 'bg-white/5 text-gray-400 border-white/10'}`}>
