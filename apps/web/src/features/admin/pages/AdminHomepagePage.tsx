      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-lg">
            Manual selections appear first. Automatic products fill every remaining position.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl flex flex-col h-[600px]">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-3">
            {selected.map((id, index) => {
              ...
              return (
                <div
                  className="bg-black/40 border border-white/10 rounded-xl p-3 flex items-center gap-4 group hover:border-white/20 transition-all"
                  key={id}
                >
                  <div className="w-8 h-8 rounded-full bg-amber-500/20 text-amber-400 flex items-center justify-center font-bold text-sm shrink-0 border border-amber-500/20">
                    {index + 1}
                  </div>
            ...
            {!selected.length ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-8 bg-black/20 rounded-xl border border-white/5 border-dashed">
                ...
                <span className="text-gray-400">
                  The backend will dynamically choose all {limit} products.
                </span>
              </div>
            ) : null}
          </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl flex flex-col h-[600px]">
          <div className="mb-6 pb-4 border-b border-white/10 shrink-0">
            ...
            <p className="text-gray-400 text-sm mt-1">
              Only currently eligible products can be selected.
            </p>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="shrink-0">
                    {isSelected ? (
                      <span className="text-xs font-bold uppercase tracking-wider text-amber-400 bg-amber-500/20 px-3 py-1.5 rounded-lg border border-amber-500/20">
                        Selected
                      </span>
                    ) : (
                      <span className="text-xs font-bold uppercase tracking-wider text-white bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg transition-colors border border-white/10">
                        Add
                      </span>
                    )}
                  </div>
      ...
      <div className="pt-8 border-t border-white/10">
        <div className="flex items-end justify-between mb-8">
          ...
          <span className="px-4 py-1.5 rounded-full bg-black/40 border border-white/10 text-sm font-medium text-gray-300">
            <span className="text-white">{preview?.manualCount ?? 0}</span> manual{' '}
            <span className="text-gray-500 mx-1">•</span>{' '}
            <span className="text-white">{preview?.automaticCount ?? 0}</span> automatic
          </span>
        </div>
