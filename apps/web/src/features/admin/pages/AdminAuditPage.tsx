      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-lg">
            Successful administrative mutations are recorded with actor and request metadata.
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 bg-black/20">
                ...
                <th className="p-4 text-xs font-bold tracking-wider text-gray-400 uppercase">
                  <div className="flex items-center gap-2">
                    <UserIcon className="w-4 h-4" /> Actor
                  </div>
                </th>
                <th className="p-4 text-xs font-bold tracking-wider text-gray-400 uppercase">
                  <div className="flex items-center gap-2">
                    <ActivityIcon className="w-4 h-4" /> Action
                  </div>
                </th>
                <th className="p-4 text-xs font-bold tracking-wider text-gray-400 uppercase">
                  <div className="flex items-center gap-2">
                    <FileTextIcon className="w-4 h-4" /> Entity
                  </div>
                </th>
                <th className="p-4 text-xs font-bold tracking-wider text-gray-400 uppercase">
                  <div className="flex items-center gap-2">
                    <MapPinIcon className="w-4 h-4" /> IP
                  </div>
                </th>
              </tr>
            </thead>
              ...
              {r.isLoading ? (
                ...
                r.data?.items.map((x) => (
                  <tr key={x.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-amber-500/20 text-amber-400 flex items-center justify-center text-xs shrink-0 border border-amber-500/20">
                    ...
                    <td className="p-4">
                      <div className="flex flex-col">
                        <span className="font-medium text-amber-300 text-sm">{x.entityType}</span>
                        <span
                          className="text-gray-500 text-xs font-mono mt-0.5 truncate max-w-[200px]"
                          title={x.entityId || ''}
                        >
                          {x.entityId ?? '—'}
                        </span>
