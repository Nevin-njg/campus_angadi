  ...
  return (
    <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
        <form className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl lg:col-span-2" onSubmit={submit}>
          <h2 className="text-xl font-bold text-white mb-6 pb-4 border-b border-white/10">Add category</h2>
          ...
          {categories.isLoading ? (
            ...
            <div className="divide-y divide-white/5">
              {(categories.data ?? []).map((category) => (
                <div className="p-5 hover:bg-white/[0.02] transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4 group" key={category.id}>
                  <div className="flex items-center gap-4">
                    {category.imageUrl ? (
                      <img src={category.imageUrl} alt="" className="w-12 h-12 rounded-lg object-cover bg-black/40 border border-white/10" />
                    ) : (
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-full border ${category.isActive ? 'bg-green-500/20 text-green-400 border-green-500/20' : 'bg-gray-500/20 text-gray-400 border-gray-500/20'}`}>
                          {category.isActive ? 'Active' : 'Hidden'}
                        </span>
                      </div>
                      <div className="text-gray-400 text-sm mt-1">
                        <span className="text-indigo-400">/{category.slug}</span> · Order: <span className="text-white">{category.displayOrder}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (window.confirm(`Are you sure you want to completely remove ${category.name}? This might break products using it.`)) remove.mutate(category.id)
                      }}
              ...
              {!categories.data?.length && !categories.isLoading && (
                <div className="p-12 text-center">
                  <strong className="text-white text-lg font-medium block mb-2">No categories yet</strong>
