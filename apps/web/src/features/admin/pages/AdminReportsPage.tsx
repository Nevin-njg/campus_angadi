    ...
    mutationFn: ({ id, status }: { id: string; status: ReportStatus }) =>
      adminPlatformApi.updateReport(id, {
        ...
        resolution: ['RESOLVED', 'DISMISSED'].includes(status)
          ? resolution[id] || 'Reviewed by the Campus Angaadi safety team.'
          : null,
      }),
    onSuccess: async () => c.invalidateQueries({ queryKey: ['admin', 'reports'] }),
  })
  
  const inputClass = "bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all placeholder:text-gray-500"
    ...
    <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-lg">Review product, seller and marketplace safety concerns.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative">
        ...
        {r.data?.items.map((x) => (
          <article className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl flex flex-col group hover:border-white/20 transition-all" key={x.id}>
            <div className="flex items-start justify-between mb-4 pb-4 border-b border-white/10">
              <div>
                <span className={`text-[10px] font-bold tracking-wider uppercase px-2 py-1 rounded-full border inline-block mb-2 ${
                  x.status === 'RESOLVED' ? 'bg-green-500/20 text-green-400 border-green-500/20' : 
                  x.status === 'DISMISSED' ? 'bg-gray-500/20 text-gray-400 border-gray-500/20' : 
                  x.status === 'IN_REVIEW' ? 'bg-amber-500/20 text-amber-400 border-amber-500/20' : 
                  'bg-red-500/20 text-red-400 border-red-500/20'
                }`}>
                  {x.status}
                </span>
                <h2 className="text-xl font-bold text-white capitalize">{x.type.replaceAll('_', ' ').toLowerCase()}</h2>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
              <div className="bg-white/[0.02] p-3 rounded-lg border border-white/5">
                <span className="block text-gray-500 text-xs uppercase tracking-wider mb-1">Target</span>
                <span className="text-white font-medium">{x.targetLabel}</span>
              </div>
              <div className="bg-white/[0.02] p-3 rounded-lg border border-white/5">
                <span className="block text-gray-500 text-xs uppercase tracking-wider mb-1">Reporter</span>
            ...
            <div className="mt-auto">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  onClick={() => m.mutate({ id: x.id, status: 'IN_REVIEW' })}
                  disabled={m.isPending}
                >
                  Mark in review
                </button>
                <button
                  className="flex-1 min-w-[100px] px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg transition-all shadow-[0_0_15px_rgba(79,70,229,0.3)]"
                  onClick={() => m.mutate({ id: x.id, status: 'RESOLVED' })}
                  disabled={m.isPending}
                >
                  Resolve
                </button>
                <button
                  className="flex-1 min-w-[100px] px-4 py-2 text-sm font-medium text-white bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition-all"
                  onClick={() => m.mutate({ id: x.id, status: 'DISMISSED' })}
                  disabled={m.isPending}
                >
                  Dismiss
                </button>
        ...
        {r.data && !r.data.items.length && !r.isLoading && (
          <div className="col-span-1 lg:col-span-2 py-16 text-center">
            ...
            <span className="text-gray-400">The marketplace is currently clear of safety concerns.</span>
          </div>
        )}
      </div>
    </section>
