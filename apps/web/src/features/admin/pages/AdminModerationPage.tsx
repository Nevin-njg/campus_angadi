    ...
    <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-lg">
            Review every second-hand product before it becomes public.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {(queue.data?.items ?? []).map((product) => (
          <article
            className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-xl hover:bg-white/[0.07] transition-all group flex flex-col sm:flex-row sm:items-center gap-6"
            key={product.id}
          >
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-center">
              <strong className="text-xl font-bold text-white mb-2 block truncate">
                {product.title}
              </strong>
              <div className="flex flex-wrap items-center gap-3 text-sm text-gray-400">
                ...
                <span className="px-2 py-1 rounded-md bg-white/5 border border-white/5 capitalize">
            ...
            <div className="flex sm:flex-col items-center sm:items-end justify-between gap-4 mt-4 sm:mt-0 shrink-0">
              ...
              <Link
                className="px-6 py-2 bg-amber-600 hover:bg-amber-500 text-white font-medium rounded-xl transition-all shadow-[0_0_15px_rgba(245,158,11,0.25)] w-full sm:w-auto text-center"
      ...
      {!queue.isLoading && !queue.data?.items.length ? (
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-16 shadow-xl flex flex-col items-center justify-center text-center">
          ...
          <span className="text-gray-400 text-lg">
            There are no products matching the selected moderation status.
          </span>
        </div>
      ) : null}
    </section>
