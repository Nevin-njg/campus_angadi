      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-lg">
            Submitted by <span className="text-amber-400">{value.sellerSnapshot.displayName}</span>.
          </p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {value.images.map((image) => (
              <div
                key={image.id}
                className="bg-black/40 border border-white/10 rounded-2xl overflow-hidden aspect-video relative group"
              >
                <img
                  src={image.url}
          ...
          <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl">
              ...
              <div className="flex flex-col gap-1 p-3 rounded-xl bg-white/[0.02] border border-white/5">
                ...
                <dd className="text-white font-medium">
                  {value.originalPrice ? (
                    ...
                    <span className="text-gray-500 italic">Not supplied</span>
                  )}
                </dd>
              </div>
              <div className="flex flex-col gap-1 p-3 rounded-xl bg-white/[0.02] border border-white/5">
                ...
                <dd className="text-white font-medium capitalize">
                  {value.condition.replaceAll('_', ' ').toLowerCase()}
                </dd>
              </div>
              <div className="flex flex-col gap-1 p-3 rounded-xl bg-white/[0.02] border border-white/5">
                <dt className="text-xs uppercase tracking-wider text-gray-500 font-medium">Age</dt>
                <dd className="text-white font-medium">
                  {value.productAge ?? <span className="text-gray-500 italic">Not supplied</span>}
                </dd>
              </div>
              <div className="flex flex-col gap-1 p-3 rounded-xl bg-white/[0.02] border border-white/5">
                <dt className="text-xs uppercase tracking-wider text-gray-500 font-medium">
            ...
            {value.reasonForSelling && (
              <div className="mt-6 pt-6 border-t border-white/10">
                ...
                <p className="text-gray-300 bg-black/20 p-4 rounded-xl border border-white/5">
                  {value.reasonForSelling}
                </p>
              </div>
            )}
              ...
              {value.moderationHistory.map((item) => (
                ...
                >
                  ...
                  <div className="bg-white/[0.02] border border-white/5 p-4 rounded-xl w-full">
                    ...
                    {item.reason && (
                      <blockquote className="text-gray-400 text-sm border-l-2 border-amber-500/50 pl-3 py-1 my-2 bg-black/20 rounded-r-lg pr-2">
                        {item.reason}
                      </blockquote>
                    )}
                    {item.actor && (
        ...
        <aside className="space-y-8 sticky top-8">
          <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/10">
              <div className="w-10 h-10 rounded-full bg-amber-500/20 text-amber-400 flex items-center justify-center border border-amber-500/20 shrink-0">
                <ShieldIcon />
              </div>
              <div>
                <h2 className="text-lg font-bold text-white">Seller</h2>
                <p className="text-xs text-amber-300 uppercase tracking-wider font-medium">
