    ...
    <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-4xl font-extrabold tracking-tight text-white mb-2">Orders</h1>
          <p className="text-gray-400 text-lg">Manage and monitor marketplace transactions.</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 bg-white/[0.02]">
                ...
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Buyer</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Type</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Status</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Dealer</th>
              ...
              {orders.data?.items.map((order) => (
                <tr key={order.id} className="hover:bg-white/[0.02] transition-colors group">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Link to={`/admin/orders/${order.id}`} className="text-indigo-400 hover:text-indigo-300 font-medium transition-colors">
                      {order.orderNumber}
                    </Link>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded border ${
                      order.sellerType === 'ADMIN' ? 'bg-purple-500/20 text-purple-300 border-purple-500/20' : 'bg-orange-500/20 text-orange-300 border-orange-500/20'
                    }`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {order.assignedDealer ? (
                      <span className="text-white font-medium">{order.assignedDealer.displayName}</span>
                    ) : (
              ...
              {orders.data && !orders.data.items.length && !orders.isLoading && (
                <tr>
                  <td colSpan={8} className="px-6 py-16 text-center">
                    ...
                    <strong className="text-white text-lg font-medium block mb-1">No matching orders</strong>
                    <span className="text-gray-400">Try adjusting your filters or search query.</span>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {orders.data && orders.data.meta.totalPages > 1 && (
          <div className="p-4 border-t border-white/10 flex items-center justify-between bg-white/[0.01]">
            ...
            <span className="text-sm text-gray-400 font-medium">
              Page <span className="text-white">{page}</span> of <span className="text-white">{orders.data.meta.totalPages}</span>
            </span>
            <button
              className="px-4 py-2 text-sm font-medium text-white bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
