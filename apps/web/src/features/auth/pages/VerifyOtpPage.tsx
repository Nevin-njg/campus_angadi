...
export function VerifyOtpPage() {
  ...
  const maskedEmail =
    (location.state as { maskedEmail?: string } | null)?.maskedEmail ?? email;
  ...
  const [resendAt, setResendAt] = useState(() =>
    Number(
      window.sessionStorage.getItem("campusbaza-resend-after") ?? Date.now(),
    ),
  );
  ...
  const submit = form.handleSubmit(async (input) => {
    ...
    try {
      ...
      void navigate(result.user.profileCompleted ? "/" : "/account/profile", {
        replace: true,
      });
    } catch (error) {
      setServerError(
        ...
        <div className="auth-visual-copy">
          ...
          <div className="auth-trust">
            ...
            <div>
              ...
              <span>
                Campus Angadi staff will never ask for this code through chat or
                a call.
              </span>
            </div>
          </div>
