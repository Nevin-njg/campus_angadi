  ...
  beforeEach(async () => {
    ...
    app.use((req, res, next) => {
      ...
      if (authHeader === 'Bearer valid-token') {
        req.auth = { user: { id: testUserId, role: 'USER', name: 'Test User', email: 'test@campus.edu', isEmailVerified: true } }
        next()
      } else if (authHeader === 'Bearer invalid-token') {
        req.auth = { user: { id: new mongoose.Types.ObjectId().toString(), role: 'USER', name: 'Other', email: 'other@campus.edu', isEmailVerified: true } }
        next()
      } else {
    ...
    app.use('/profile', createProfileRouter(profileService, (req, res, next) => next()))
